...
class ProfileController
{
    public function updateAvatar(Request $request){
        ...
        if($request->hasFile('avatar')){
            ...
            Image::make($avatar)->resize(300,300)->save( public_path( $newAvatarPath) );
...
/*
	if(ctype_alpha($ime) == false || ctype_alpha($prezime) == false){
			$nameflag = -1;
	}
*/
        ...
        if( $nameflag == 0 && $phoneflag == 0 && $passflag == 0 && $emailflag == 0 && $opisflag == 0) {
            $user = Auth::user();
	    ...
	    $user->firstname = $ime;
            ...
            return "Promene nisu izvrsene. Lozinke se ne podudaraju";
        }
        else if($emailflag == -1){
            return "Email je nevalidan. Zahtev nije ispunjen. " . $emailString;
        }
        else if($opisflag == -1){
            return "Greska u opisu. Zahtev nije ispunjen.";
        }
	else if($nameflag == -1){
		return "Greska u imenu i/ili prezimenu. Zahtev nije ispunjen.";
	}
