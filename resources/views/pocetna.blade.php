@extends('layout')

@section('pocetna')

<!--
<br/>
    <div class="row">
    <a href="/pretraga?faks=ETF&smer=SI&godina=2&predmet=ASP2"><button id="traka" class="btn btn-primary ribbon col-xs-10 col-xs-offset-1 col-md-6 col-md-offset-3" style="font-size:1.2em;">Algoritmi i strukture podataka 2</button></a>
    <br/>
    <br/>
	<br/><br/>
        <div class="col-xs-12 col-xs-offset-0 col-md-6 col-md-offset-3">
            <div class="box">
                <div class="box-icon">
                    <span class="fa fa-4x fa-television"></span>
                </div>
                <div class="info">
                    <h4 class="text-center">Prvi domaći zadatak</h4>
			     <p class="text-center">Smer : <span style="color:black">Softversko inženjerstvo (SI)</span></p>
                    <p class="text-center">Postavka zadatka : <a id="link" href="http://rti.etf.bg.ac.rs/rti/ri3sp/domaci/13S112ASP2_DZ1_1617.pdf">ASP2 I domaći</a></p>

                    <p class="text-center">Oblast : <span style="color:black">Binarna stabla pretaživanja (BST)</span></p>
                              
  
                  
                  
                    <a href="/pretraga?faks=ETF&smer=SI&godina=2&predmet=ASP2" class="btn  btn-block btn-primary" style="width:100%; font-size:1.2em;">Potraži pomoć</a>
                </div>
            </div>
        </div>
        
	</div>
    -->   
<style>
.carousel-control.left,.carousel-control.right  {background:none;width:25px;}
.carousel-control.left {left:-25px;}
.carousel-control.right {right:-25px;}
.broun-block {
    background: transparent repeat scroll center top rgba(0, 0, 0, 0);
    padding-bottom: 34px;
}
.block-text {
    background-color:white;
    border-radius: 5px;
 //   box-shadow: 0 3px 0 #2c2222;
    //color: black;
    font-size: 14px;
    margin-top: 27px;
    padding: 15px 18px;
height:240px;
}
.block-text a {
    color: #006dcc;
    font-size: 25px;
    font-weight: bold;
    line-height: 21px;
    text-decoration: none;
    text-shadow: 0 1px 2px rgba(0, 0, 0, 0.3);
}
.mark {
    padding: 12px 0;background:none;
}
.block-text p {
    color: #585858;
    font-family: Georgia;
    font-style: italic;
    line-height: 20px;
}
.sprite-i-triangle {
    background-position: 0 -1298px;
    height: 44px;
    width: 50px;
}
.block-text ins {
    bottom: -44px;
    left: 50%;
    margin-left: -60px;
}


.block {
    display: block;
}
.zmin {
    z-index: 1;
}
.ab {
    position: absolute;
}

.person-text {
    padding: 10px 0 0;
    text-align: center;
    z-index: 2;
}
.person-text a {
    color: yellow;
    display: block;
    font-size: 14px;
    margin-top: 3px;
    text-decoration: underline;
}
.person-text i {
    color: #fff;
    font-family: Georgia;
    font-size: 13px;
}
.rel {
    position: relative;
}
.sprite {
	background-image: url('data:image/png;base64,');
}
</style> 
<div class="carousel-reviews broun-block">
        <div class="row">
            <div id="carousel-reviews" class="carousel slide" data-ride="carousel">
            
                <div class="carousel-inner">
                    <div class="item active">
                	    <div class="col-md-4 col-sm-6">
        				    <div class="block-text rel zmin">
						        <a title=""  href="/profile/22">Dario Mirovic</a>
<p>Moju pomoc u izradi projekata iz OS1 u poslednjih nekoliko godina potrazilo je preko 200 ljudi. Nikad nije bilo problema sa provjerom slicnosti. Mogu sve za predmet da pomognem, gradivo za kolokvijume, za ispit, projekat. Na vama je da ispostujete moja uputstva za nacin ucenja. Drzim i druge predmete. Obzirom da sam se zaposlio nedavno, broj kolega sa kojima radim je ogranicen.</p>
							    <ins class="ab zmin sprite sprite-i-triangle block"></ins>
					        </div>
							<div class="person-text rel">
								<img style="width:75px;75px;" alt="" src="/uploads/avatars/default-avatar.png">							


								<a title=""  href="/profile/22">Dario Mirovic</a>
								<i>Operativni sistemi</i>
							</div>
						</div>
            			<div class="col-md-4 col-sm-6">
						    <div class="block-text rel zmin">
						        <a title=""  href="/profile/35">Vaso Djeric</a>

<p>Prvi čas iz OET1,održao sam 7.08.2010.-e godine. Cilj je isti,položiti ispit u prvom predstojećem roku,uraditi odlično kolokvijum,ako se spremamo za kolokvijum.Student je aktivan učesnik na času,rešava zadatke,odgovara na postavljena pitanja,biva podstaknut da razmišlja,sve u cilju razvijanja logike,preko potrebne za polaganje OET1.</p>
					            <ins class="ab zmin sprite sprite-i-triangle block"></ins>
				            </div>
							<div class="person-text rel">
							<img style="width:75px;height:75px;" src="uploads/avatars/1477001321.jpg" class="img-circle" />
						        <a title=""  href="/profile/35">Vaso Djeric</a>
							<i>OET i elektronika</i>							
</div>
						</div>
						<div class="col-md-4 col-sm-6">
							<div class="block-text rel zmin">
								<a title=""  href="/profile/32">Zdravko Nedic</a>
<p>Držim časove iz preko 20 računarskih predmeta za ETF od 2005. godine. Do sada je uz moju pomoć oko 1500 studenata položilo ispite. Moguć je rad u grupama. Preporuke možete pregledati na forumu ETF-a.</p>
    
								<ins class="ab zmin sprite sprite-i-triangle block"></ins>
							</div>

							<div class="person-text rel">
							<img style="width:75px;height:75px;" src="uploads/avatars/1476906540.jpg" class="img-circle" />
						        <a title=""  href="/profile/32">Zdravko Nedic</a>

								<i>Master inženjer elektrotehnike</i>
							</div>
						</div>
                    </div>
                    <div class="item">
                        <div class="col-md-4 col-sm-6">
        				    <div class="block-text rel zmin">
						        <a title=""  href="/profile/30">Алекса Марушић</a>
<p>Сва потребна помоћ око предмета. 
Изврсно познавање градива. 
Искуство у помагању људима да спреме многе предмете.</p>
							    <ins class="ab zmin sprite sprite-i-triangle block"></ins>
					        </div>
							<div class="person-text rel">
								<img style="width:75px;75px;" alt="" src="/uploads/avatars/default-avatar.png">							

								<a title=""  href="/profile/30">Алекса Марушић</a>
								<i>Студент друге године СИја</i>
							</div>
						</div>
            			<div class="col-md-4 col-sm-6">
						    <div class="block-text rel zmin">
						        <a title=""  href="/profile/11">Danilo Rakonjac</a>
<p>Treca sam godina studija fizike na Fizickom fakultetu (9.75 prosek), a spremao sam ljude i ranije za ispite iz fizike, tako da mogu da spremim bilo koga da to polozi sa zeljenom ocenom. </p>
					            <ins class="ab zmin sprite sprite-i-triangle block"></ins>
				            </div>
							<div class="person-text rel">
								<img style="width:75px;75px;" alt="" src="/uploads/avatars/default-avatar.png">							
						        <a title=""  href="/profile/11">Danilo Rakonjac</a>
								<i>Olimpijac iz fizike</i>
							</div>
						</div>
						<div class="col-md-4 col-sm-6">
							<div class="block-text rel zmin">
								<a title="" href="/profile/21">Ivan Ilic</a>

<p>Sva potrebna pomoc oko programiranja. 
Iskustvo u radu sa velikim brojem kolega prethodnih godina koji su bez problema odradili kolokvijume i polozili ispite.</p>	
							<ins class="ab zmin sprite sprite-i-triangle block"></ins>
							</div>
							<div class="person-text rel">
								<img style="width:75px;75px;" alt="" src="/uploads/avatars/default-avatar.png">
								<a title="" href="/profile/21">Ivan Ilic</a>
								<i>Programiranje</i>
							</div>
						</div>
                    </div>
                </div>
                <a class="left carousel-control" href="#carousel-reviews" role="button" data-slide="prev">
                    <span class="glyphicon glyphicon-chevron-left"></span>
                </a>
                <a class="right carousel-control" href="#carousel-reviews" role="button" data-slide="next">
                    <span class="glyphicon glyphicon-chevron-right"></span>
                </a>
            </div>
        </div>
    </div>

@endsection
